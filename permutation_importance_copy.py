...
class Chain(BaseEstimator):
    ...
    def importances_target(self, X, y_true, y_pred):
        ...
        v = X.shape[1]
        feat_imp_score = []
        for a in range(1, y_pred.shape[1] + 1):  # Adjusted range
            #print(a)
            y_true_a = y_true.iloc[:, a-1]  # Adjusted indexing
            y_pred_a = y_pred.iloc[:, a-1]  # Adjusted indexing
            feat_imp_score_model = []
            for i in range(v+a-1):  # Adjusted range
                #print(i)
                Xext = data.iloc[:, :a+v-1].copy()  # Copying relevant columns
                #print(Xext.dtypes)
                last = Xext.iloc[:, -1]
                ...
                if i < v:  # Adjusted condition
                    ...
                    if last.dtype == 'object':
                        ...
                        yi_pred_num = pd.Series(yi_pred_num)#.replace(-1, np.nan, inplace=True)
                        Xext.iloc[:, -1] = yi_pred_num
                    for m in range(i-v, a-1):  # Adjusted range
                        yi_pred = model_list[m+1].predict(Xext)
                        #print(yi_pred)
                        if yi_pred.dtype == 'object':
                            #yi_pred = np.array(yi_pred)
                            yi_pred_num = pd.Categorical(yi_pred).codes
                            #print(yi_pred_num)
                            yi_pred_num = pd.Series(yi_pred_num)#.replace(-1, np.nan, inplace=True)
                            #print(yi_pred_num)
                            ...
                            #print(Xext[column_name])
                        else:
                            column_name = data.iloc[:, m+v+1].name
                            Xext[column_name] = yi_pred
                        Xext_ori[column_name] =  yi_pred
                        #print(Xext_ori[column_name]) 
                    yi_pred_shuffled = Xext_ori.iloc[:, -1] 
                    #print(yi_pred_shuffled)

                assert len(y_true_a) == len(yi_pred_shuffled)
                missing_rows_mask = pd.Series(y_true_a).notna()  # Corrected method
                ...
                if y_test.dtype.kind in 'bifc':
                    ...
                    score2 = accuracy_score(y_test, y_pred_shuffled_actual)  # Changed to accuracy score
