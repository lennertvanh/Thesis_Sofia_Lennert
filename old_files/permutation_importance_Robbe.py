...
class Chain(BaseEstimator):
    ...
    def importances_target(X, y_true, y_pred):
        if not isinstance(X, pd.DataFrame):
            X = pd.DataFrame(X, columns=[f"X{i}" for i in range(X.shape[1])])
        Xext = X.copy()
        pred_list = X.copy()
        ...
        for i in range(y_pred.shape[1]):
            # 1. shuffle column in y_pred_shuffled
            # (...)
            # 2. regenerate predictions for remaining models
            for j in range(i+1, y_pred.shape[1]):
                # Use sklearn permutation feature importance test on model j
                yi_pred = model.predict(Xext)
                if yi_pred.dtype == 'object':
                    yi_pred_num = pd.Categorical(yi_pred).codes
                    yi_pred_num = pd.Series(yi_pred_num).replace(-1, np.nan, inplace=True)
    ...
    chain = Chain().fit(Xtr,ytr)
    y_pred = chain.predict(Xtest)
    X_importance = # should be easy
    y_importance = 