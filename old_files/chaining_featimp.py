    ...
    def predict(self, X, y):
        ...
        for col, model in zip(self.y_columns, self.models):
            if self.propagate == False:
                ...
                perm_imp=permutation_importance(model, X, yi_pred, n_repeats=10, random_state=0)
                perm_imp=perm_imp.importances_mean
                feat_imp_list.append(perm_imp)
            else:
                yi_pred = model.predict(Xext)
                if yi_pred.dtype == 'object':
                    yi_pred_num = pd.Categorical(yi_pred).codes
                    yi_pred_num = pd.Series(yi_pred_num).replace(-1, np.nan, inplace=True)
                    perm_imp=permutation_importance(model, Xext, yi_pred, n_repeats=10, random_state=0)
                    perm_imp=perm_imp.importances_mean
                    feat_imp_list.append(perm_imp)
        ...
        return [pred_list.iloc[:, -len(self.models):], feat_imp_list]